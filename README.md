...
El dominio solo conoce interfaces; las adaptaciones (red, BD, UI) se inyectan mediante dependencias, lo que mejora testabilidad y flexibilidad.
...
Con `expect/actual` (en `Platform.kt`), se definen contratos comunes en `commonMain` y se implementan detalles nativos en `androidMain` e `iosMain`.
