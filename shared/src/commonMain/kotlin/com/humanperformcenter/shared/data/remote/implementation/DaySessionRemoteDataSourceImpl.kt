...
class DaySessionRemoteDataSourceImpl(
    ...
    : Result<ReserveUpdateResponse> = runCatching {
        clientProvider.apiClient.patch(
            "${clientProvider.baseUrl}/mobile/bookings/${reserveUpdateRequest.booking_id}"
        ) {
            contentType(ContentType.Application.Json)
    ...
    override suspend fun getUserProductId(customerId: Int): Result<Int> = runCatching {
        val products = clientProvider.apiClient.get(
            "${clientProvider.baseUrl}/mobile/users/$customerId/products"
        ).body<List<Product>>()
        products.first().id
    }
    ...
    override suspend fun getProductServiceInfo(productId: Int): Result<Int> = runCatching {
        clientProvider.apiClient.get(
            "${clientProvider.baseUrl}/mobile/product/$productId/service-info"
        ).body()
    }

    override suspend fun getTimeslotId(serviceId: Int, dayOfWeek: String, hour: String)
    : Result<Int> = runCatching {
        val payload = clientProvider.apiClient.get(
            "${clientProvider.baseUrl}/mobile/timeslot-id"
        ) {
