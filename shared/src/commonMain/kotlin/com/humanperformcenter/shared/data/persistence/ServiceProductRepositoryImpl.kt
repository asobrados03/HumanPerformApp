...
object ServiceProductRepositoryImpl: ServiceProductRepository {
    ...
    override suspend fun assignProductToUser(
        ...
        couponCode: String?
    ): Result<Int> { // Devolvemos el ID asignado o Fallo
        ...
        return try {
            ...
            if (response.status.isSuccess()) {
                ...
                val mensajeAmigable = when (response.status.value) {
    ...
    override suspend fun unassignProductFromUser(userId: Int, productId: Int): Result<Unit> {
        return try {
            ...
            if (response.status.isSuccess()) {
                ...
                val errorBody = try { response.body<SimpleResponse>() } catch (e: Exception) { null }
    ...
    override suspend fun applyCoupon(code: String, userId: Int, productId: Int): Result<SimpleResponse> {
        return try {
            val response = ApiClient.apiClient.post("${ApiClient.baseUrl}/mobile/apply-coupon") {
                contentType(ContentType.Application.Json)
                setBody(CouponApplyRequest(code, userId, productId))
            }
            ...
            if (response.status.isSuccess() && body.success) {
                ...
                // Si el backend envía success: false o un error HTTP
                Result.failure(Exception(body.error ?: "Cupón no válido"))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
